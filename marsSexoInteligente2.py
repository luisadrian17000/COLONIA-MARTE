...
PEATON_Z_VALUE = 0
EXPLORERS_Z_VALUE = 1
N_DEFENSE_SYSTEMS = 900
N_SOLAR_PANELS = 100
N_ALIEN_PLANTS = 308
N_FOOD_TRUCK = 324
N_SPACE_EQUIPMENT = 10
N_PUBLIC_LAMP = 100
...
class DefenseSystems(ap.Agent):

    def setup(self):
        valid_positions = np.argwhere(city_grid == 1).tolist()
        if not valid_positions:
            raise ValueError("No valid spawn points (1) found!")
        px = random.choice(valid_positions)
        valid_positions.remove(px) #Eliminar la posicion para no volver a usarla.
        self.x, self.y = map(int, px)
        self.defense_number = np.random.randint(1, 5)

class SolarPanels(ap.Agent):

    def setup(self):
        self.solar_panels_id = self.id
        valid_positions = np.argwhere(city_grid == 1)
...
class SpaceEquipment(ap.Agent):
    ...
    def setup(self):
        self.space_equipment_id = self.id
        valid_positions = np.argwhere(city_grid == 1).tolist()  # Convertir a lista para poder eliminar elementos

        if not valid_positions:
            raise ValueError("No valid spawn points (1) found!")

        px = random.choice(valid_positions)  # Seleccionar una posicion aleatoria
        valid_positions.remove(px)  # Eliminar la posicion seleccionada para no volver a usarla.
        self.x, self.y = map(int, px)
...
class PublicLamp(ap.Agent):
    ...
    def setup(self):
        ...
        self.lamp_number = np.random.randint(1, 5)
...
class CityModel(ap.Model):
    def setup(self):
        ...
        self.defense_systems = ap.AgentList(self, N_DEFENSE_SYSTEMS, DefenseSystems)
        self.solar_panels = ap.AgentList(self, N_SOLAR_PANELS, SolarPanels)
        self.alien_plants = ap.AgentList(self, N_ALIEN_PLANTS, AlienPlants)
        self.food_trucks = ap.AgentList(self, N_FOOD_TRUCK, FoodTruck)
        self.space_equipment = ap.AgentList(self,N_SPACE_EQUIPMENT, SpaceEquipment)
        self.public_lamp = ap.AgentList(self, N_PUBLIC_LAMP, PublicLamp)
        ...
        self.all_agents += self.defense_systems
        self.all_agents += self.solar_panels
        self.all_agents += self.alien_plants
        self.all_agents += self.food_trucks
        self.all_agents += self.space_equipment
        self.all_agents += self.public_lamp
...
anuncio_positions = []
defensa_positions = []
solar_panel_positions = []
alien_plants_positions = []
food_truck_positions = []
space_equipment_positions = []
public_lamp_positions = []
...
for _ in range(N_STEPS):
    ...
    defensa_positions.append([(a.x, a.y) for a in model.anuncios])
    solar_panel_positions.append([(a.x, a.y) for a in model.anuncios])
    alien_plants_positions.append([(a.x, a.y) for a in model.anuncios])
    food_truck_positions.append([(a.x, a.y) for a in model.anuncios])
    space_equipment_positions.append([(a.x, a.y) for a in model.anuncios])
    public_lamp_positions.append([(a.x, a.y) for a in model.anuncios])
model.end()
# --- Animación ---
...
scat_defensa = ax.scatter([], [], c='yellow', s=120, label='Anouncements')
scat_solar = ax.scatter([], [], c='blue', s=120, label='Anouncements')
scat_alien = ax.scatter([], [], c='green', s=120, label='Anouncements')
scat_food = ax.scatter([], [], c='gray', s=1200, label='Anouncements')
scat_space = ax.scatter([], [], c='brown', s=12000, alpha=0.5, label='Anouncements')
scat_lamp = ax.scatter([], [], c='orange', s=1000, alpha=0.2, label='Anouncements')
...
def update(frame):
    ...
    positions_defenseSystems = defensa_positions[frame] #correccion
    if defensa_positions:
        x_defensa, y_defensa = zip(*positions_defenseSystems)
    else:
        x_defensa, y_defensa = [], []
    scat_defensa.set_offsets(np.c_[y_defensa, x_defensa])

    positions_solarSystems = solar_panel_positions[frame] #correccion
    if solar_panel_positions:
        x_solar, y_solar = zip(*positions_solarSystems)
    else:
        x_solar, y_solar = [], []
    scat_solar.set_offsets(np.c_[y_solar, x_solar])

    positions_alienPlants = alien_plants_positions[frame] #correccion
    if alien_plants_positions:
        x_alien, y_alien = zip(*positions_alienPlants)
    else:
        x_alien, y_alien = [], []
    scat_alien.set_offsets(np.c_[y_alien, x_alien])

    positions_foodTrucks = food_truck_positions[frame] #correccion
    if food_truck_positions:
        x_food, y_food = zip(*positions_foodTrucks)
    else:
        x_food, y_food = [], []
    scat_food.set_offsets(np.c_[y_food, x_food])

    positions_spaceEquipments = space_equipment_positions[frame] #correccion
    if space_equipment_positions:
        x_space, y_space = zip(*positions_spaceEquipments)
    else:
        x_space, y_space = [], []
    scat_space.set_offsets(np.c_[y_space, x_space])

    positions_lamp = public_lamp_positions[frame] #correccion
    if public_lamp_positions:
        x_lamp, y_lamp = zip(*positions_lamp)
    else:
        x_lamp, y_lamp = [], []
    scat_lamp.set_offsets(np.c_[y_lamp, x_lamp])

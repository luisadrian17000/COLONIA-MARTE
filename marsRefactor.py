#En este codigo los agentes anuncios ya no se solapan

# Mejor implementacion del algoritmo Q-learning
...
class Advertisement2(ap.Agent):

    def setup(self):
        self.anouncement2_id = self.id
        ...
        self.advertisement2_number = np.random.randint(1,5)


    def step(self):
        self.advertisement_number2 = np.random.randint(1,5)
...
class CityModel(ap.Model):
    def setup(self):
        ...
        self.anuncios2 = ap.AgentList(self, N_ANUNCIOS2, Advertisement2)
        ...
        self.all_agents += self.anuncios2
    ...
    def step(self):
        ...
        for anuncio2 in self.anuncios2:
            anuncio2.step()
            anuncio2.action()
...
for _ in range(N_STEPS):
    ...
    anuncio2_positions.append([(a.x, a.y) for a in model.anuncios2])
model.end()
# --- Animación ---
...
scat_anouncements2 = ax.scatter([], [], c='brown', s=120, label='Anouncements2')
...
def update(frame):
    ...
    positions_anouncements2 = anuncio2_positions[frame]  # correccion
    if positions_anouncements2:
        x_anouncements2, y_anouncements2 = zip(*positions_anouncements2)
    else:
        x_anouncements2, y_anouncements2 = [], []
    scat_anouncements2.set_offsets(np.c_[y_anouncements2, x_anouncements2])
